...
def test_flash_image_drawing():
    ...
    # 测试1：直接使用Gemini-2.5-Flash-Image生成
    print("=" * 70)
    print("测试1：直接使用Gemini-2.5-Flash-Image生成")
    print("=" * 70)
    ...
    output_file = f"flash_image_direct_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    print(f"📁 输出文件: {output_file}")
    print()
    
    try:
        result = tool.create_drawing_with_flash_image(request, output_file)
        ...
        if result and os.path.exists(output_file):
            ...
            print(f"📄 文件格式: PNG")
            print(f"🎨 AI引擎: Google Gemini-2.5-Flash-Image")
            print()
        else:
            print("❌ 生成失败")
    ...
    print("\n" + "=" * 70)
    print("测试2：Gemini + Gemini-2.5-Flash-Image 增强绘图")
    print("=" * 70)
    
    output_file2 = f"flash_image_enhanced_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
