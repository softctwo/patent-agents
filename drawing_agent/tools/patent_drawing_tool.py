...
class PatentDrawingTool:
    ...
    def _draw_element(
        ...
        """绘制单个元素"""
        # 获取样式
        stroke_width = self.DEFAULT_LINE_WIDTH * 2  # 转换为像素
        ...
        if element.element_type == "border":
            ...
            w = element.width * self.DEFAULT_DPI / 25.4
            h = element.height * self.DEFAULT_DPI / 25.4
            ...
            x = element.position["x"] * self.DEFAULT_DPI / 25.4
            y = element.position["y"] * self.DEFAULT_DPI / 25.4
            r = element.radius * self.DEFAULT_DPI / 25.4
